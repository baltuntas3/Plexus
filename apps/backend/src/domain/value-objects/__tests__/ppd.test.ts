...
describe("computePPD", () => {
  ...
  it("rejects a zero-cost baseline", () => {
    expect(() =>
      computePPD({ accuracy: 0.8, costUsd: 1 }, { accuracy: 0.8, costUsd: 0 }),
    ).toThrow(/baseline.costUsd/);
  });

  it("rejects a zero-accuracy baseline", () => {
    expect(() =>
      computePPD({ accuracy: 0.8, costUsd: 1 }, { accuracy: 0, costUsd: 1 }),
    ).toThrow(/baseline.accuracy/);
  });

  it("rejects accuracy outside [0,1]", () => {
    expect(() =>
      computePPD({ accuracy: 1.2, costUsd: 1 }, { accuracy: 0.8, costUsd: 1 }),
    ).toThrow(/candidate accuracy/);
    expect(() =>
      computePPD({ accuracy: 0.8, costUsd: 1 }, { accuracy: -0.1, costUsd: 1 }),
    ).toThrow(/baseline accuracy/);
  });

  it("rejects negative cost", () => {
    expect(() =>
      computePPD({ accuracy: 0.8, costUsd: -1 }, { accuracy: 0.8, costUsd: 1 }),
